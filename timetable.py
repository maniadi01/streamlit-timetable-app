...
# Initialize worksheets with caching and batching
@st.cache_resource
    ...
    try:
        ...
        for sheet_name, headers in required_headers.items():
            if sheet_name not in existing_sheets:
                ...
                if not ws.get_all_values() or not ws.get_all_values()[0]:
...
# Validate Timetable row
def validate_timetable_row(row, assign_missing_id=False):
    try:
        # Check required fields (allow Timetable ID to be assigned if missing)
        ...
        # Validate Start Time and End Time format (e.g., "Monday 10:00")
        ...
        # Assign new Timetable ID if missing and allowed
...
def clean_timetables_worksheet(sheet, assign_missing_ids=False):
    ...
    try:
        ...
        valid_rows = [list(data[0].keys())]  # Keep headers
        invalid_count = 0
        invalid_rows = []
        for row in data:
...
# Load data from Google Sheets with retry and validation
    ...
    for attempt in range(2):
        try:
            ...
            def normalize_columns(df, expected_cols):
                ...
                if missing_cols:
                    st.error(f"Missing columns in {df.name}: {missing_cols}")
                    return None
            ...
            faculty_df = normalize_columns(faculty_df, ["School ID", "School Name", "Department ID", "Department Name", "Faculty ID", "Faculty Name"])
            rooms_df = normalize_columns(rooms_df, ["Department ID", "Room Name"])
            subjects_df = normalize_columns(subjects_df, ["Subject Name", "Subject ID"])
            
            # Validate and filter Timetables data
            valid_timetables = []
            invalid_count = 0
            ...
            if invalid_count > 0:
                st.warning(f"Skipped {invalid_count} invalid Timetables rows. Use 'Clean Invalid Timetables' to remove them permanently.")
            
            timetables_df = pd.DataFrame(valid_timetables)
            timetables_df.name = "Timetables"
            timetables_df = normalize_columns(timetables_df, ["Timetable ID", "Date", "Department ID", "Room Name", "Faculty ID", "Subject ID", "Start Time", "End Time"])
...
# Save timetable to Google Sheets with retry and validation
...
def auto_assign_timeslots(faculty_assignments, dept_rooms, avoid_friday_afternoon=False, max_attempts=20):
    schedule = []
    days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]
    ...
    if total_classes * max(assignment["tenure"] for assignment in faculty_assignments) > len(days) * len(time_slots) * len(dept_rooms):
    ...
    for assignment in faculty_assignments:
            ...
            for day in shuffled_days:
                ...
                for time in shuffled_times:
                    ...
                    if faculty_key in faculty_slots:
                        failures.append(f"Faculty {faculty_id} already scheduled at {time} on {day}.")
                        continue
        ...
        if classes_assigned < num_classes:
            st.warning(f"Could not schedule {num_classes - classes_assigned} classes for {subject_id} (Faculty: {faculty_id}). Reasons: {set(failures) or ['Unknown constraints']}. Try adding more rooms or adjusting time slots.")
...
def create_timetable_page(sheet):
    ...
    with st.spinner("Loading data..."):
        faculty_df, rooms_df, subjects_df, _ = load_data(sheet)
    ...
    st.subheader("Basic Information ℹ")
    ...
    for idx, assignment in enumerate(st.session_state.temp_assignments[:]):
        with st.container():
            ...
            with col6:
                if st.button("🗑", key=f"remove_{idx}_{st.session_state.widget_counter}"):
                    ...
                    for class_idx in range(num_classes):
                        ...
                        with col1:
                            day = st.selectbox(
                                "Day bulging",
                                options=days,
                                key=f"day_{faculty_id}{subject_id}{class_idx}_{st.session_state.widget_counter}"
                            )
                        with col2:
                            time = st.selectbox(
                                "Time",
                                options=time_slots,
                                key=f"time_{faculty_id}{subject_id}{class_idx}_{st.session_state.widget_counter}"
                            )
                        with col3:
                            if pre_assigned_room:
                                ...
                                room = st.selectbox(
                                    "Room",
                                    options=dept_rooms["Room Name"].tolist(),
                                    key=f"room_{faculty_id}{subject_id}{class_idx}_{st.session_state.widget_counter}"
                                )
    ...
    if submitted:
        ...
        if auto_assign:
            with st.spinner("Generating timetable automatically..."):
                try:
                    schedule = auto_assign_timeslots(st.session_state.faculty_assignments, dept_rooms, avoid_friday_afternoon)
                except Exception as e:
...
def previous_timetables_page(sheet):
    ...
    if st.button("🧹 Clean Invalid Timetables", key=f"clean_timetables_{st.session_state.widget_counter}"):
        with st.spinner("Cleaning Timetables worksheet..."):
            invalid_count, invalid_rows = clean_timetables_worksheet(sheet, assign_missing_ids=True)
            if invalid_count > 0 and invalid_rows:
                st.warning(f"Removed {invalid_count} invalid Timetables rows. Download invalid rows for review?")
                if st.button("Download Invalid Rows CSV", key="download_invalid_rows_previous"):
                    invalid_df = pd.DataFrame(invalid_rows)
                    csv = invalid_df.to_csv(index=False)
                    st.download_button(
                        label="Download Invalid Rows",
                        ...
                        file_name="invalid_timetables.csv",
                        mime="text/csv",
                        key="download_invalid_csv_previous"
                    )
            load_data.clear()
            st.rerun()
    
    if timetables_df is None or timetables_df.empty:
        st.info("No valid timetables found. Try cleaning the Timetables worksheet if errors persist.")
        return
